	...
	if ($employees) { ## Checking to see if there was actually any employees in the CSV
		foreach ($employee in $employees)
		{
			try
			{
				...
				if (Invoke-Command @icmParams -ScriptBlock { Get-ADUser -Filter "Name -eq '$using:userName'"})
				{
				...
				else
				{
					...
					if (-not (Invoke-Command @icmParams -ScriptBlock { Get-ADOrganizationalUnit -Filter "DistinguishedName -eq '$using:departmentOuPath'"})) {
					...
					Invoke-Command @icmParams -ScriptBlock { New-AdUser @using:newUserParams }
				}

				## Adding to department group

				## Check to see if the group exists
				if (-not (Invoke-Command @icmParams -ScriptBlock { Get-ADGroup -Filter "Name -eq '$($using:employee.Department)'"})) {
					throw "Unable to find the group [$($employee.Department)]"
				} else {
					## If so, check to see if the user is already a member
					$groupMembers = Invoke-Command @icmParams -ScriptBlock { (Get-ADGroupMember -Identity $using:employee.Department).Name }
					if (-not ($userName -in $groupMembers)) {
						...
						Invoke-Command @icmParams -ScriptBlock { Add-ADGroupMember -Identity $using:employee.Department -Members $using:userName }
					} else {
...
## Disable any accounts in AD that are not in this CSV
	...
	foreach ($emp in $nonEmployeeAdUsers) {
		Invoke-Command @icmParams -ScriptBlock { $using:emp | Disable-AdAccount }
	}